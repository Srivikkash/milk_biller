...
class design(vars):
    def __init__(self,window):
        ...
        btn3 = Button(self.window,text="view history",command=self.history)
    ...
    def clicked(self):
        ...
        try:
            ...
            Label(self.window,text ="In Valid entry pls try again",).place(x=0,y=255)
    ...
    def run(self):
        ...
        try:
            ...
            self.error_win = Toplevel()
            Label(self.error_win,text=" error Enter your calculation and submit it to save").grid(column=0,row=0)
            btn = Button(self.error_win,text="ok",command=self.close_win).grid(column=1,row=2)
        #write the history list
        writer_ob = writer(cv)
        writer_ob.writerow(var)
        self.window3 = Toplevel()
        self.window3.title("Notify")
        Label(self.window3,text='History Saved',font='bold').grid(column=0,row=0)
        Button(self.window3,text="Done",command=self.close_win).grid(column=0,row=2)
        cv.close()
    ...
    def history(self):
        ...
        self.window2.title("HISTORY")
        his = cv.readlines()
        data = list(his)
        lenth = len(his)
        Label(self.window2,text=his).grid(column=0,row=1)
        Button(self.window2,text="close",command=self.close_win).grid(column=0,row=lenth+1)
